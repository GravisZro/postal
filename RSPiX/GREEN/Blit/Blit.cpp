...
//    goto PLOT_DONTLOCK;
  ...
  switch(reinterpret_cast<intptr_t>(pimDst->m_pSpecial)) // 0 = normal image
  {
...
//PLOT_DONTLOCK:
  ...
  // DO THE PLOT!
  PIXSIZE* pTemp = reinterpret_cast<PIXSIZE*>(pimDst->m_pData + sY * pimDst->m_lPitch) + sX;
  *pTemp = color;
...
void instantiatePlot()
{
  ...
  rspPlot<uint8_t >(0, pim, 0, 0);
  rspPlot<uint16_t>(0, pim, 0, 0);
  rspPlot<uint32_t>(0, pim, 0, 0);
}
...
//
template <class PIXSIZE>
int16_t rspLasso(PIXSIZE ignoreColor,
                 RImage* pimSrc,
                 int16_t& x, int16_t& y,
                 int16_t& w, int16_t& h,
                 int lFlag,
                 ...
                 int bFlag)
{
  ...
  if (lFlag)
  {
    pCursorLine = reinterpret_cast<PIXSIZE*>(pimSrc->m_pData) + lSkipV * y + x;

    for (i = w; i > 0; --i, ++x, --w, ++pCursorLine)
      for (pCursor = pCursorLine, j = 0; j < h; ++j, pCursor += lSkipV)
...
DoneL:
  ...
  if (rFlag)
  {
    pCursorLine = reinterpret_cast<PIXSIZE*>(pimSrc->m_pData) + lSkipV * y + (x + w - 1);

    for (i = w; i > 0; --i, --w, --pCursorLine)
      for (pCursor = pCursorLine, j = 0; j < h; ++j, pCursor += lSkipV)
        if (*pCursor != ignoreColor)
...
DoneR:
  ...
  if (tFlag)
  {
    ...
    for ( i = h; i > 0; --i, ++y, --h, pCursorLine += lSkipV)
      for (pCursor = pCursorLine, j = 0; j < w; ++j, ++pCursor)
        if (*pCursor != ignoreColor)
...
DoneT:
  ...
  if (bFlag)
  {
    pCursorLine = reinterpret_cast<PIXSIZE*>(pimSrc->m_pData) + lSkipV * (y + h - 1) + x;

    for (i = h; i > 0; --i, --h, pCursorLine -= lSkipV)
      for (pCursor = pCursorLine, j = 0; j < w; ++j, ++pCursor)
        if ( (*pCursor) != ignoreColor )
...
// force instantiation
template int16_t rspLasso<uint8_t >(uint8_t  ignoreColor, RImage* pimSrc, int16_t& x, int16_t& y, int16_t& w, int16_t& h, int lFlag, int rFlag, int tFlag, int bFlag);
template int16_t rspLasso<uint16_t>(uint16_t ignoreColor, RImage* pimSrc, int16_t& x, int16_t& y, int16_t& w, int16_t& h, int lFlag, int rFlag, int tFlag, int bFlag);
template int16_t rspLasso<uint32_t>(uint32_t ignoreColor, RImage* pimSrc, int16_t& x, int16_t& y, int16_t& w, int16_t& h, int lFlag, int rFlag, int tFlag, int bFlag);
...
template <class PIXSIZE>
inline void _BLiT(uint8_t* pSrcBytes,
                  uint8_t* pDstBytes,
                  int32_t lSrcPitch,
                  int32_t lDstPitch,
                  int16_t sHeight,
                  int16_t sByteWidth)
{
  ...
  PIXSIZE* pSrc = reinterpret_cast<PIXSIZE*>(pSrcBytes);
  PIXSIZE* pDst = reinterpret_cast<PIXSIZE*>(pDstBytes);
...
// "garatied SegBus"... did you mean "guaranteed SIGBUS"?
inline void _BLiT_MA(uint8_t* pSrc,
                     uint8_t* pDst,
                     int32_t lSrcPitch,
                     int32_t lDstPitch,
                     int16_t sHeight,
                     int16_t sWidth)
{
#if defined(__ARM_NEON__) || defined(__x86_64__)
  while (sHeight--)
  {
...
// 
int16_t rspBlitA(RImage* pimSrc,
                 RImage* pimDst,
                 ...
                 const RRect* prSrc)
{
  ...
  int16_t sBestAlign = clChosenAlignment | pimSrc->m_lPitch | pimDst->m_lPitch;
...
//
int16_t rspBlit(RImage* pimSrc,
                RImage* pimDst,
                int16_t sSrcX, int16_t sSrcY,
...
*/
  ...
  int16_t sAlign = sSrcX | sDstX | sW | ABS(pimSrc->m_lPitch) | ABS(pimDst->m_lPitch);
  ...
  {
    // 128-bit copy
    _BLiT<uint128_t>(pSrc, pDst, pimSrc->m_lPitch, pimDst->m_lPitch, sH, sW);
  }
  else if ( (sAlign & 7) == 0)
  {
    // 64-bit copy
    _BLiT<uint64_t>(pSrc, pDst, pimSrc->m_lPitch, pimDst->m_lPitch, sH, sW);
  }
  else if ( (sAlign & 3) == 0)
  {
    // 32-bit copy
    _BLiT<uint32_t>(pSrc, pDst, pimSrc->m_lPitch, pimDst->m_lPitch, sH, sW);
  }
  else if ( (sAlign & 1) == 0)
  {
    // 16-bit copy
    _BLiT<uint16_t>(pSrc, pDst, pimSrc->m_lPitch, pimDst->m_lPitch, sH, sW);
  }
  else
  {
    // 8-bit copy
    ...
    if (sW < 16)
      _BLiT<uint8_t>(pSrc,pDst,pimSrc->m_lPitch,pimDst->m_lPitch,sH,sW);
    else
...
inline void _ClearRect(WORDSIZE color, uint8_t* pDstBytes, int32_t lDstPitch,
                       int16_t sHeight, int16_t	sByteWidth)
{
  ...
  WORDSIZE* pDst = reinterpret_cast<WORDSIZE*>(pDstBytes);
  pDstLine.w = pDst;
...
*/
  ...
  if ( (sAlign & 3) == 0)
  {
    ...
    _ClearRect<uint32_t>(ulColor, pDst, pimDst->m_lPitch, sH, sByteW);
  }
  else if ( (sAlign & 1) == 0)
  {
    // 16-bit copy
    uint16_t usColor = (uint16_t)(color + (color << 8)); // 16-bit;
    _ClearRect<uint16_t>(usColor, pDst, pimDst->m_lPitch, sH, sByteW);
  }
  else
  {
    // 8-bit copy
    _ClearRect<uint8_t>(color, pDst, pimDst->m_lPitch, sH, sByteW);
  }
...
//
// Modify current:
int16_t rspPad(RImage* pimSrc,
               int16_t sX, int16_t sY, // where to move the old image to
