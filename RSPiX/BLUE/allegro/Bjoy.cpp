...
///////////////////////////////////////////////////////////////////////////////
//
// Handles all Dreamcast specific joystick stuff.
//
//////////////////////////////////////////////////////////////////////////////
...
//static SDL_Joystick* ms_Joysticks[NUM_JOYSTICKS];
...
{
/*
  if (SDL_Init(SDL_INIT_JOYSTICK) != SUCCESS)
    return;
  SDL_JoystickEventState(SDL_IGNORE);

  const int sticks = SDL_NumJoysticks();
  int opened = 0;
  for (int i = 0; i < sticks && opened < NUM_JOYSTICKS; i++)
  {
    ms_Joysticks[opened] = SDL_JoystickOpen(i);
    if(ms_Joysticks[opened] != nullptr)
      ++opened;
  }
*/
}
...
extern void GetDudeVelocity(double* d_Velocity, double* d_Angle)
{
  ...
  *d_Velocity = sqrt(pow(axis_MoveUpDown, 2) + pow(axis_MoveLeftRight, 2)) / 32768.f;
  ...
  //TRACE("DudeVel X%i Y%i Vel %f Angle %f", (int)axis_MoveLeftRight, (int)axis_MoveUpDown, *d_Velocity, *d_Angle);
}
...
extern bool GetDudeFireAngle(double* d_Angle)
{
  ...
  //TRACE("DudeFire TRUE X%i Y%i Angle %f", (int)axis_FireLeftRight, (int)axis_FireUpDown, *d_Angle);
  return true;
}
