...
// Startup object
////////////////////////////////////////////////////////////////////////////////
void CHood::Startup(void)								// Returns 0 if successfull, non-zero otherwise
   {
	}
  ...
  UNUSED(sX, sY, sZ);
	...
	if (pguiEdit != nullptr)
		...
		if (pguiBaseName != nullptr)
         ...
         RSP_SAFE_GUI_REF(pguiBrowse, m_ulUserInstance = reinterpret_cast<uintptr_t>(pguiBaseName));
	...
	if (rspGetResource(
      ...
      ppspry) == SUCCESS)
		...
		if (type != RImage::NOT_SUPPORTED)
			{
			// Convert . . .
         if ((*ppspry)->Convert(type) == SUCCESS)
				{
				// Success.
				}
			else
